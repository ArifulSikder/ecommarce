...
class CagegoryController extends Controller
{
    ...
    function updateCategory(Request $request){
        ...
        $validator = Validator::make($request->all(), [
            'category_name' => 'required|max:255|unique:categories,category_name,'.$request->input('id'),
            'category_icon' => 'required|max:255',
            'category_slug' => 'required|max:255|unique:categories,slug,'.$request->input('id'),
            'category_thumbnail' => 'required|max:255',
            'category_status' => 'required|max:11',
        ]);
        ...
        $category = Category::findOrFail($request->input('id'))->update([
            ...
            'category_thumbnail' => $category_thumbnail,
            'slug' =>  strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $request->input('category_slug')))),
        ]);
